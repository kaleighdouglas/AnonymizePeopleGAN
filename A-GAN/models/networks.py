...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[], mask_input=False, netG_noise='none', stage_1=True, diff_map=False):
    """Create a generator
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetGenerator(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=mask_input, netG_noise=netG_noise, stage_1=stage_1, diff_map=diff_map)  #### ADDED for input 64x64
    # elif netG == 'unet_up_64':
    #     net = UnetGeneratorUpsample(input_nc, output_nc, 6, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=mask_input, netG_noise=netG_noise, stage_1=stage_1, diff_map=diff_map)  #### ADDED for input 64x64
    elif netG == 'unet_128':
        net = UnetGenerator(input_nc, output_nc, 7, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=mask_input, netG_noise=netG_noise, stage_1=stage_1, diff_map=diff_map)  #### CHANGED added gpu_ids, stage_1, diff_map
    elif netG == 'unet_256':
        net = UnetGenerator(input_nc, output_nc, 8, ngf, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=mask_input, netG_noise=netG_noise, stage_1=stage_1, diff_map=diff_map)  #### CHANGED added gpu_ids, stage_1, diff_map
...
class UnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, gpu_ids=[], mask_input=False, netG_noise='none', stage_1=True, diff_map=False): #input_nc=3, output_nc=3, num_downs=8, ngf=64
        ...
        if netG_noise == 'none':
            ...
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, innermost=True, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)  # add the innermost layer   #### CHECK submodule w/ sequential
            for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
                unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            # gradually reduce the number of filters from ngf * 8 to ngf
            unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            unet_block = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            unet_block = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=mask_input, stage_1=stage_1, diff_map=diff_map)  # add the outermost layer

        elif netG_noise == 'encoder_one':
            # construct unet structure
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, innermost=True, addrandomnoise='none', norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)  # add the innermost layer   #### CHECK submodule w/ sequential
            for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
                unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, addrandomnoise='none', norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            # gradually reduce the number of filters from ngf * 8 to ngf
            unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block, addrandomnoise='none', norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            unet_block = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None, submodule=unet_block, addrandomnoise='none', norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            unet_block = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, addrandomnoise='none', norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=mask_input, stage_1=stage_1, diff_map=diff_map)  # add the outermost layer

        elif netG_noise == 'encoder_all':
            # construct unet structure
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None, innermost=True, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)  # add the innermost layer   #### CHECK submodule w/ sequential
            for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
                unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            ...
            unet_block = UnetSkipConnectionBlock(ngf, ngf * 2, input_nc=None, submodule=unet_block, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=False, stage_1=stage_1)
            #self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, addrandomnoise=netG_noise, norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=mask_input, stage_1=stage_1, diff_map=diff_map)  # add the outermost layer
            self.model = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, addrandomnoise='none', norm_layer=norm_layer, gpu_ids=gpu_ids, mask_input=mask_input, stage_1=stage_1, diff_map=diff_map)  # add the outermost layer
...
class UnetSkipConnectionBlock(nn.Module):  ### ORIGINAL VERSION
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, addrandomnoise='none', norm_layer=nn.BatchNorm2d, use_dropout=False, gpu_ids=[], mask_input=False, stage_1=True, diff_map=False):  #### CHECK BatchNorm2d
        ...
        ## NOISE LENGTH
        if addrandomnoise == 'encoder_one' or addrandomnoise == 'encoder_all':
            noise_len = 8
            input_nc += noise_len
        elif addrandomnoise == 'decoder':
            if innermost:
                noise_len = 256
            else:
                noise_len = 128
        ...
        if outermost: #and stage_1:
            ...
            if addrandomnoise == 'none':
                ...
                # up = [uprelu, upconv]
                # noise = [addnoise, noiseconv]
                # model = down + noise + [submodule] + up

            elif addrandomnoise == 'encoder_one' or addrandomnoise == 'encoder_all':
            ...
            elif addrandomnoise == 'encoder_all':
                ## Noise before encoder
                # noise_len = 8 #256 #128
                addnoise = AddNoise(noise_len, gpu_ids=gpu_ids)
            ...
            if addrandomnoise == 'none':
                ...
                ## ADDED NOISE WITH CONV (after down)
                # noise_len = 128 #256 #128
                # addnoise = AddNoise(noise_len, gpu_ids=gpu_ids)
                ...
                # noise = [addnoise, noiseconv]
                # if use_dropout:
                #     model = down + noise + [submodule] + up + [nn.Dropout(0.5)]
                # else:
                #     model = down + noise + [submodule] + up
    ...
    def forward(self, input):
        ...
        if self.outermost:
            if self.diff_map:     ## DIFFERENCE MAP VERSION
                ...
                out_combined = out.add(torch.atanh(input[:,:3,:,:]))
...
#### Network for person discriminator -- conv network with GAP and BATCH SPP
class GAP_SPP_NET(nn.Module):    #### CHANGE hardcoded BatchNorm2d
    def __init__(self, input_nc, ndf=64, norm_layer=nn.BatchNorm2d):
        super(GAP_SPP_NET, self).__init__()
    ...
    def forward(self, x, bbox=[]):
        ...
        c1 = self.conv1(x)
        a1 = self.LReLU1(c1)
        # print('conv1', c1.size())  #15

        c2 = self.conv2(a1)
        a2 = self.LReLU2(self.BN1(c2))
        # print('conv2', c2.size())  #14

        c3 = self.conv3(a2)
        a3 = self.LReLU3(self.BN2(c3))
        # print('conv3', c3.size())  #13
        ...
        g = self.GAP(c4)
        gap_a4 = self.GELU(g)
        gap_a4 = torch.squeeze(gap_a4)
        gap_out = self.FC(gap_a4)
        # print('gap_out size',gap_out.size())
        ...
        if c5.size()[0] == 1:
            spp = self.spatial_pyramid_pool(c5, 1, [int(c5.size(2)), int(c5.size(3))], self.output_num)
            # print('spp', spp.size())
            gap_out = gap_out.unsqueeze(0)
            # print('gap_out', gap_out.size())
            gap_spp = torch.cat([gap_out, spp], dim=1)
            return gap_spp, (c1, c2, c3, c4, g) #, c5, spp)

        #### If batch size > 1, iterate through batch to call spp on each img  ####
        ## Padded height & width
        padded_conv_height = c5.size()[2]
        padded_conv_width = c5.size()[3]

        for i in range(c5.size()[0]):
            x1, y1, x2, y2 = bbox
...
class SPP_NET(nn.Module):    #### CHANGE hardcoded BatchNorm2d
    ...
    def forward(self, x, bbox=[]):
        ...
        cout = self.conv5(a4)
        # print('conv5', cout.size())  #9   #### must be at least 10 to avoid error
        # print('conv out', cout)

        ## If batch size == 1, call spp directly
        if cout.size()[0] == 1:
            spp = self.spatial_pyramid_pool(cout, 1, [int(cout.size(2)), int(cout.size(3))], self.output_num)
            # print('spp', spp.size())
            return spp, (c1, c2, c3, c4, cout) #, spp)

        #### If batch size > 1, iterate through batch to call spp on each img  ####
        ## Padded height & width
        padded_conv_height = cout.size()[2]
        padded_conv_width = cout.size()[3]

        for i in range(cout.size()[0]):
            x1, y1, x2, y2 = bbox
            ...
            x_crop = torch.unsqueeze(cout[i, :, yc1:yc2, xc1:xc2], 0)
            # print('x_crop', x_crop.size())
